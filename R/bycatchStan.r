...
#' @param modeledEffort TRUE if effort is drawn from a normal
...
#' @param stanObj One rstan or cmdstanr object
...
getResiduals<-function(stanSum,stanObj,modelNum,setupObj,nsim=1000,spNum=1,useCode) {
  ...
  if(useCode=="rstan") b0vals <- extract(stanObj, pars = "b0")$b0
  if(useCode=="cmdstanr") b0vals <- stanObj$draws(variables="b0",format="df")$b0
  subsetval <- sample(1:length(b0vals), nsim)
  matrix1<-model.matrix(formula(stanSum$waictab$Model[modelNum]),data=obsdat)
  if(ncol(matrix1) > 1) {
      if(useCode=="rstan") bvals <- extract(stanObj, pars = "b")$b
      if(useCode=="cmdstanr") bvals <- stanObj$draws(variables="b",format="matrix")
      bvals <- bvals[subsetval, ]
  } else
      bvals <- NULL
  b0vals <- b0vals[subsetval]
  bvals <- cbind(b0vals, bvals)
  if(useCode=="rstan") phivals <- extract(stanObj, pars = "phi")$phi[subsetval]
  if(useCode=="cmdstanr") phivals <- stanObj$draws(variables="phi",format="df")$phi[subsetval]
...
# Extract draws of the bycatch estimates
#' getBycatchDraws.
#'
#' @param stanSum Output from bycatchStanSim
#' @param stanObj One rstan or cmdstanr object
#' @param modelNum Model number corresponding to rows in waictab
#' @param setupObj Data check output from BycatchEstimator dataCheck
#' @param modeledEffort TRUE if effort is input as mean and se for simulation.
#' @param effortSD   Name of column with effort standard deviation
#' @param useCode rstan or cmdstanr
#'
...
getBycatchDraws<-function(stanSum,
  ...
  postvals<-getBycatchSim(stanObj,
                          ...
                          returnDraws=TRUE,
                          useCode=useCode)[[2]]
  postSum<-postvals %>% group_by_at(all_of(c(requiredVariables,"iterations"))) %>%
    summarize(bycatch=sum(simVal))
}



#Function to get mortality predictions from fitted bycatch and mortality models applied logdat
...
getMortPred <- function(stanSum,
  ...
  modelyrSum1 <- gg1 %>% group_by_at(all_of(c(summaryVariables,"iterations"))) %>%
    ...
    pivot_longer(Bycatch:Mortality,
                 names_to = "Outcome",
                 values_to = "Total") %>%
    group_by_at(all_of(c(summaryVariables, "Outcome"))) %>%
    summarize(
      mean = mean(Total),
      se = sd(Total),
